...
<center>
$$\huge{\mathbb{E}[e^{-rT}(S_T-K)^+]=e^{-rT}\int_{\mathbb{R}}\frac{1}{\sqrt{2 \pi}}(S_T \exp{r-\frac{1}{2}\sigma^2 T + \sigma \sqrt{T}x}-K)^+ \exp{-x^2/2}dx
}$
</center>
...
<center>
S_T \exp{r-\frac{1}{2}\sigma^2 T + \sigma \sqrt{T}x}-K \geq 0 //
x \geq \frac{log(K/S_T-(r-T\frac{\sigma^2}{2}))}{\sigma \sqrt{T}} = d_1
</center>

So we can get rid of the maximum term in the integrand and further simplify the expression as

<center>
$$\huge{\mathbb{E}[e^{-rT}(S_T-K)^+]=e^{-rT}\int_{d_1}^{\infty}\frac{1}{\sqrt{2 \pi}}(S_T \exp{r-\frac{1}{2}\sigma^2 T + \sigma \sqrt{T}x}-K) \exp{-x^2/2}dx=\\
S_T \int_{d_1}^{\infty}\frac{1}{\sqrt{2 \pi}} \exp{\frac{(x-\sigma\sqrt{T})^2}{2}}dx - e^{-rT} K \int{d_1}^{\infty}\frac{1}{\sqrt{2 \pi}} e^{-x^2/2}dx
}$
</center>
