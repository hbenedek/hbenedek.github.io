...
For a movie released in year \\( x \\) $\large x$, we look for quotes containing the title (or subtitle) of the movie in year \ x \ and \ x + 1 \. In this way we will have information about the hype before the publish date and see the afterlife as well. The filtering happened using in a different python script, we just load the json files containing the results.
